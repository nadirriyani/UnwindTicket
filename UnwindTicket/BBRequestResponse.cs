...
namespace UnwindTicket
{
    class BBRequestResponse
    {
        ...
        public  List<List<string>> GetBbFields(string[] tickers, string[] fieldsParam)
        {
            ...
            var sessionOptions = new SessionOptions();  //new SessionOptions { ServerHost = serverHost, ServerPort = serverPort };

            var session = new Session(sessionOptions);
            session.StartAsync(); //start session with async
